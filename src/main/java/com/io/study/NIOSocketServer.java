...
public class NIOSocketServer {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        /**
         * 对于使用的线程池技术，我一定要多说几句
         * 1、Executors是线程池生成工具，通过这个工具我们可以很轻松的生成“固定大小的线程池”、“调度池”、“可伸缩线程数量的池”
         * 2、当然您也可以通过ThreadPoolExecutor直接生成池。
         * 3、这个线程池是用来得到操作系统的“IO事件通知”的，不是用来进行“得到IO数据后的业务处理的”。要进行后者的操作，您可以再使用一个池（最好不要混用）
         * 4、您也可以不使用线程池（不推荐），如果决定不使用线程池，直接AsynchronousSocketChannel.open()就行了
         */
        ...
        // 并不包括为随后客户端和服务器socketchannel通道注册的监听
...
class ServerSocketChannelHandle implements CompletionHandler<AsynchronousSocketChannel, Void> {
    ...
    public void completed(AsynchronousSocketChannel socketChannel, Void attachment) {
        ...
        // 每次都要重新注册监听（一次注册，一次响应），但是由于“文件状态标示符”是独享的，所以不需要担心有“漏掉”的事件
        ...
        socketChannel.read(readBuffer, new StringBuffer(), new SockectChannelReadHandle(socketChannel, readBuffer));
    }
 ...
 *
 *  但是最重要的，用于生产channel的对象：AsynchronousChannelProvider是单例模式，无论在哪组socketchannel，
 *  对象是一个对象引用（但这没关系，因为您不会直接操作这个AsAsynchronousChannelProvider对象
 */
class SockectChannelReadHandle implements CompletionHandler<Integer, StringBuffer> {

    private static final Log LOGGER = LogFactory.getLog(SockectChannelReadHandle.class);

    private AsynchronousSocketChannel socketChannel;

    /**
     * 专门用于进行这个通道数据缓存操作的ByteBuffer
     * 当然，您也可以作为CompletionHandler的attachment形式传入
     * 这是，在这段示例代码中，attachement被我们用来记录所有传送过来的StringBuffer了。
     */
    private ByteBuffer byteBuffer;

    public SockectChannelReadHandle(AsynchronousSocketChannel socketChannel, ByteBuffer byteBuffer) {
    ...
    public void completed(Integer result, StringBuffer historyContext) {
        ...
        /**
         * 实际上，由于我们从Integer result知道了本次channel从操作系统获取数据长度
         * 所以实际上，我们不需要切换成“读模式”的，但是为了保证编码的规范性，还是建议进行切换。
         *
         * 另外，无论是JAVA AIO框架还是JAVA NIO框架，都会出现“buffer的总容量” 小于 “当前从操作系统获取的总数据量”，
